...
// --- JOGOSULTSÁG ELLENŐRZŐ (MEGOSZTÁSHOZ) ---
async function checkReadAccess(req, targetUserId) {
  if (targetUserId === req.userId) return true; // Saját adathoz mindig van jog
  
  const [rows] = await pool.query(
    'SELECT 1 FROM shares WHERE owner_id = ? AND shared_with_email = ?',
...
app.get('/api/invoices', verifyUser, async (req, res) => {
  ...
  if (!(await checkReadAccess(req, targetUserId))) {
...
app.get('/api/assets', verifyUser, async (req, res) => {
  ...
  if (!(await checkReadAccess(req, targetUserId))) {
    return res.status(403).json({ error: 'Nincs jogosultságod ehhez az adathoz' });
  }
...
app.post('/api/assets', verifyUser, async (req, res) => {
  ...
  try {
    ...
    console.error("SQL HIBA AZ ESZKÖZMENTÉSNÉL:", err);
